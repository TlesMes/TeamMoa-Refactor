...
| 뷰 | AS-IS 쿼리 수 | TO-BE 쿼리 수 | 쿼리 감소율 | 비고 |
|---|------------|-----------|--------|------|
| **TeamMembersPageView** | 16개 (N=5) | 4개 | **75%** | 가장 큰 개선 |
| **MindmapDetailView** | 6개 | 2개 | **67%** | prefetch 적용 |
| **PostListView** | 4개 | 2개 | **50%** | select_related |

### **확장성 테스트 시뮬레이션**

> ⚠️ **실측이 아닌 산술 추정이다.** 템플릿 필터링이 팀원 수 × 할일 수 만큼
> 비교하던 것을 직접 접근으로 바꿨을 때의 연산 횟수를 계산한 값이며,
> 응답 시간을 측정한 것이 아니다. 실측 응답 시간은 위 표와 `troubleshooting.md` 를 볼 것.

| 팀 규모 | 할일 개수 | AS-IS 연산 | TO-BE 연산 | 연산 횟수 비 (산술) |
|---------|-----------|------------|------------|--------|
| 5명 | 50개 | 250회 비교 | 직접 접근 | **250배** |
| 10명 | 100개 | 1,000회 비교 | 직접 접근 | **1,000배** |
| 20명 | 500개 | 10,000회 비교 | 직접 접근 | **10,000배** |

### **시간 복잡도 개선** *(분석적 결론, 실측 아님)*
- **템플릿 필터링**: O(N×M) → O(N) *(M배 — 연산 횟수 기준)*
