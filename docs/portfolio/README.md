...
flowchart TD
    subgraph Client["Client Layer"]
        Browser["Web Browser<br/>(SSR + JS)"]
        APIClient["API Client<br/>(REST/JSON)"]
    end

    subgraph Presentation["Presentation Layer"]
        Views["Django Views<br/>(SSR)<br/>- Form 처리<br/>- 템플릿 렌더링"]
        ViewSets["DRF ViewSets<br/>(API)<br/>- JSON 응답<br/>- Serializer"]
    end

    subgraph Service["Service Layer"]
        BusinessLogic["Business Logic<br/>(services.py)<br/><br/>TeamService.create_team()<br/>TodoService.assign_todo()<br/>ScheduleService.get_team_availability()<br/>MindmapService.create_node()"]
    end

    subgraph Data["Data Layer"]
        ORM["Django ORM<br/>(models.py)"]
        DB[("MySQL 8.0<br/>Database")]
    end

    Browser -->|HTTP Request| Views
    APIClient -->|HTTP Request| ViewSets
    Views -->|Both use| BusinessLogic
    ViewSets -->|Both use| BusinessLogic
    BusinessLogic --> ORM
    ORM --> DB
...
**💡 핵심 원칙**: View/ViewSet은 HTTP만 처리, Service는 비즈니스 로직만 담당
