...
**각 단계 시작 직전에 서버 세션에 알린다.** 서버 쪽 자원 수집기를
같은 시간대에 띄워야 CPU/메모리/DB 커넥션이 부하 곡선과 맞물린다.
스테이지 이름은 `vu0050`, `vu0100`, `vu0200`, `vu0400`, `vu0800` 으로
정확히 맞춰야 집계 시 병합된다.
...
서버 세션이 보내준 `server_*.csv` 를 `results/` 에 넣고:
