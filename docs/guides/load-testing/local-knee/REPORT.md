...
12스레드 머신에서 앱 프로세스 하나가 꽉 찬 동안 머신에는 여유가 남아 있었다.

> **한계는 하드웨어가 아니라 배포 구성이다.**
> VU 50에서 88%, VU 90에서 127%(천장)에 닿았고, 무릎이 정확히 거기 있다.

> ⚠️ **정확히 하자면**: 호스트 CPU 25~36%(= 3.0~4.3코어) 중 컨테이너 몫은
> 약 1.5코어(web 1.27 + DB 0.24)다. 나머지 1.5~2.8코어는 앱과 무관한
> 호스트 부하였을 가능성이 크다 — 이후 측정에서 브라우저와 게임이
> 코어 3개를 쓰고 있는 것을 확인했다(2026-09-11 04:18). 다만 1차 측정
> 시점에는 그 기록을 남기지 않아 정확한 내역은 알 수 없다.
>
> **이 불확실성은 1차의 결론을 바꾸지 않는다.** 최소 7코어 이상이 비어
> 있었고, 앱 프로세스가 그 여유를 전혀 쓰지 못한 채 127%에서 멈췄다는
> 사실은 그대로다. 호스트가 포화된 적이 없으므로 앱은 호스트에 막히지
> 않았다. (반면 2차는 호스트가 89~94%였으므로 이 오염에 직접 영향을
> 받았고, 그래서 2차는 호스트를 비운 상태로 재측정한다.)
>
> 이 일 이후 수집기가 측정 시작 시 호스트 프로세스별 CPU를 기록하고
> 무관한 부하가 0.3코어를 넘으면 경고하도록 했다
> (`host_processes_<stage>.txt`).
