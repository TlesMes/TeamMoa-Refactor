...
대역폭 — 서버 PC에 iperf3 3.21 설치 완료 (`winget install --id ar51an.iPerf3`).
노트북에도 같은 버전을 설치한다.
...
```bash
# 노트북 — 양방향 모두 잰다
iperf3 -c 192.168.50.97 -t 20        # 업로드: 요청 트래픽
iperf3 -c 192.168.50.97 -t 20 -R     # 다운로드: 응답 트래픽 (병목 판정의 핵심)
```

> winget 설치 직후에는 PATH가 현재 셸에 반영되지 않는다. 셸을 새로 열거나 전체 경로로 실행:
> `%LOCALAPPDATA%\Microsoft\WinGet\Packagesr51an.iPerf3_*\iperf3.exe`

**기록**: RTT 평균/최대/표준편차, 양방향 대역폭(Mbps), TCP 재전송(Retr) 수.

판정 기준: 목표 RPS × 평균 응답 크기가 `-R` 대역폭의 절반을 넘으면 네트워크가 병목이다.
응답이 요청보다 훨씬 크므로(공유게시판 8.7KB) `-R` 방향이 결정적이다.
재전송 수가 눈에 띄면 무선 간섭이 있다는 뜻이고, 그 상태의 p95는 서버 특성이 아니라
무선 특성을 반영한다.
