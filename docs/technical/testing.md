...
| 앱 | 주요 검증 항목 |
|----|----------------|
| Teams | 팀 생성/가입/해체, 마일스톤 CRUD, 마일스톤-TODO 연동 |
| Members | TODO 드래그앤드롭, 상태 변경, 마일스톤 할당 |
| Schedules | 주간 스케줄, 팀 가용성 계산 |
| Shares | 게시판 CRUD, 검색, 파일 처리, 탈퇴 작성자 처리 |
| Accounts | 회원가입, 이메일 인증, 회원 탈퇴 |
