...
> **AWS ALB + Multi-AZ 인프라의 실제 성능을 Locust를 통해 검증**

### 테스트 개요

**목적**:
1. ALB 로드밸런싱이 실제로 2대 EC2에 트래픽을 균등 분산하는지 확인
2. 점진적 부하 증가 시 시스템 안정성 및 확장성 검증
3. SLA 목표 달성 여부 확인 (95%ile < 500ms, 에러율 < 1%)

**테스트 환경**:
- AWS ALB + EC2 t3.micro × 2대 (ap-northeast-2a, ap-northeast-2c)
- Docker Compose (Django 5.2.4 + Nginx 1.25 + MySQL 8.0 + Redis 7.0)
- HTTPS (Let's Encrypt SSL, 도메인: https://teammoa.shop)

**테스트 전략**:
- **도구**: Locust (오픈소스 부하 테스트 프레임워크)
- **일시**: 2025년 12월 16일
- **방법**: 점진적 부하 증가 (20명 → 50명 → 100명 → 150명)
...
**📌 이미지 삽입 위치: infrastructure.md 성능 차트**
```
파일명: infrastructure_load_test_summary.png
위치: 여기 (4회 테스트 비교 차트)
내용: 동시 접속자별 RPS, 응답시간, 에러율 추이
```

**핵심 성과**:

| 지표 | 목표 (SLA) | 측정값 (로그인 제외) | 달성 여부 |
|------|----------|-----------------|----------|
...
| 회차 | 동시 접속자 | 총 요청 수 | RPS | 평균 응답 시간 | 95%ile | 에러율 |
|-----|-----------|----------|-----|-------------|-------|-------|
...
> 💡 **핵심 발견**: 20명→150명 부하 증가 시 평균 응답 시간 43ms→70ms (63% 증가), 95%ile은 65ms→76ms (17% 증가)로 **선형적이고 안정적인 확장성** 확인
...
**결과**:
- 2대 EC2에 트래픽이 **거의 균등하게 분산** (49.8% vs 50.2%)
- ALB Round Robin 알고리즘 정상 작동 확인

#### 2. Health Check 안정성 ✅

**결과**:
- 4회 테스트 모두 Health Check 엔드포인트 **0% 에러율**
- 150명 고부하 환경에서도 `/health/` 응답 시간 평균 43ms 유지
- ALB Target Group Health 상태 항상 "healthy" 유지
...
**분석**:
- 100명까지 거의 동일한 성능 유지 (40~52ms)
- 150명에서도 70ms로 **SLA 목표(500ms) 대비 86% 향상**
- t3.micro × 2대로도 150명 동시 접속 충분히 처리 가능

#### 4. 에러 처리 및 복구 ⚠️

**Test #4 (150명) 에러 분석**:
- **총 에러**: 190건 / 29,047건 = **0.65%** (SLA 목표 1% 이내)
- **원인**: t3.micro 인스턴스 리소스 한계 (vCPU 2개, RAM 1GB)
- **영향**: 전체 엔드포인트에 고르게 분산 (0.5~0.9%), 특정 병목 없음
- **Health Check는 0% 에러** - ALB 헬스체크는 정상 작동

**개선 방안** (필요 시):
- 인스턴스 업그레이드 (t3.small: vCPU 2개 → 4개)
- Auto Scaling Group 도입 (CPU > 70% 시 자동 확장)
...
**✅ 성공한 점**:
1. **SLA 목표 초과 달성**: 95%ile 70ms (목표 대비 86% 향상)
2. **ALB 정상 작동 확인**: 로드밸런싱, Health Check 모두 정상
3. **선형 확장성 확인**: t3.micro × 2대로도 150명 처리 가능
4. **실전 경험**: 실제 프로덕션 환경에서 부하 테스트 수행

**📋 개선 고려 사항**:
1. **모니터링 강화**: Sentry 연동, CloudWatch 알람 설정
2. **Auto Scaling**: 트래픽 증가 시 자동 확장 (비용 대비 효과 검토 필요)

> 📄 **상세 리포트**: [부하 테스트 전체 결과](../guides/load-testing/load-test-report.md)
