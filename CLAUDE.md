...
| 페이지 | Before 쿼리 수 | Before 중복 | Before 응답 시간 |
|--------|---------------|-------------|------------------|
| **팀 메인** | 13개 | 7 duplicates | 3.7 ms |
| **멤버 보드** | 15개 | 8 similar, 4 duplicates | 4.8 ms |
| **마인드맵 목록** | 10개 | 5 similar, 3 duplicates | 3.23 ms |
| **마인드맵 상세** | 11개 | 4 similar, 4 duplicates | 3.41 ms |

**✅ 완료된 최적화**:
- **팀 메인 페이지**: 13개→8개 (38% 감소), 3.7ms→2.25ms (39% 단축) ⚡

**⏳ 진행 중**:
- **멤버 보드, 마인드맵 페이지**: After 측정 예정

**🔍 추가 발견된 문제 (향후 처리)**:

**1. N+1 쿼리 문제 - 마인드맵 상세 페이지**
- **문제**: 노드 연결(Line) 하나당 2개의 추가 쿼리 발생
- **원인**: Line 모델에서 `from_node`, `to_node` FK 접근 시 개별 조회
- **해결 방안**: `select_related('from_node', 'to_node')` 적용 필요
- 우선순위: Medium

**2. N+1 쿼리 문제 - 공유 게시판 목록 페이지** ⚠️
- **문제**: 게시물 개수에 비례하여 쿼리 증가
  - 게시물 0개: 8 쿼리
  - 게시물 1개: 10 쿼리
  - 게시물 2개: 11 쿼리
  - 게시물 3개: 12 쿼리
  - **증가율**: 게시물 +1개당 쿼리 +1~2개
- **원인**: 게시물 조회 시 작성자(author) FK 개별 조회 추정
- **해결 방안**: `select_related('author')` 적용 필요
- 우선순위: High (게시물이 많을수록 심각)

**3. N+1 쿼리 문제 - 노드 상세 페이지** ⚠️
- **문제**: 댓글 개수에 비례하여 쿼리 증가
- **원인**: 댓글 조회 시 관련 FK 접근 시 개별 조회 추정 (이미 `select_related('user')` 적용됨)
- **해결 방안**: 템플릿에서 추가 FK 접근 확인 필요
- 우선순위: Medium
