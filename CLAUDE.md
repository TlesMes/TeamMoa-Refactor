...
### ⚡ 쿼리 최적화 진행 중 (2026.01.09)

**✅ 중복 쿼리 문제 해결 완료**:
- ✅ **Mixin 캐싱**: `TeamMemberRequiredMixin`에서 Team 객체 캐시 저장
- ✅ **View 캐시 재사용**: `get_object()` 오버라이드로 중복 조회 방지
- ✅ **JOIN 최적화**: `select_related('host').prefetch_related('members')` 적용
- ✅ **COUNT 쿼리 제거**: `list()` 사용으로 템플릿 COUNT 쿼리 제거
- ✅ **Python 필터링**: 조건별 SELECT 쿼리 통합 (1개 절약)

**📊 최적화 결과** (2026.01.09 완료):
...
- ✅ FK 필드 ID 직접 사용으로 불필요한 JOIN 제거

**✅ N+1 쿼리 문제 해결 완료** (2026.01.10):

**1. 공유 게시판 목록 - 템플릿 태그 N+1 해결** ⚡
- **문제**: 게시물 +1개당 쿼리 +1~2개 증가 (게시물 3개 시 12 쿼리)
- **원인**: `{% user_display_name %}` 템플릿 태그에서 TeamUser.exists() 쿼리 반복 실행
- **해결**: 템플릿에서 `post.teamuser.user.nickname` 직접 접근
- **결과**: 게시물 3개 시 8 쿼리 (33% 감소), 항목 증가해도 쿼리 수 일정

**2. 마인드맵 상세 - Line N+1 해결** ⚡
- **문제**: Line N개 시 9+2N 쿼리 발생
- **원인**: `from_node`, `to_node` FK 개별 조회
- **해결**: `select_related('from_node', 'to_node')` 적용
- **결과**: Line N개 시 9 쿼리 (2N개 쿼리 제거)

**3. 노드 상세 - 댓글 N+1 해결** ⚡
- **문제**: 댓글 N개 시 8+N 쿼리 발생
- **원인**: `{% user_display_name %}` 템플릿 태그에서 TeamUser.exists() 쿼리 반복 실행
- **해결**: 템플릿에서 `comment.user.nickname` 직접 접근
- **결과**: 댓글 N개 시 8 쿼리 (N개 쿼리 제거)

**📊 핵심 성과**:
- **템플릿 태그 N+1 패턴 발견**: 서비스 레이어 최적화만으로는 부족, 템플릿 레이어까지 확인 필요
- **완전한 N+1 제거**: 목록 페이지에서 항목 수에 관계없이 쿼리 수 일정하게 유지
- **ORM 최적화 완성**: 서비스 레이어(`select_related`) + 템플릿(직접 접근) 조합

**📚 참고 문서**:
- [troubleshooting.md - 중복 쿼리 문제](./docs/technical/troubleshooting.md#9-중복-쿼리-문제-mixin-view-계층-간-이중-조회)
- [troubleshooting.md - 템플릿 태그 N+1 문제](./docs/technical/troubleshooting.md#10-템플릿-태그로-인한-n1-쿼리-문제)
...
7. **포트폴리오 문서 작성** - ✅ 완료 (9개 문서, 150+ 코드, 2025.12.08)
