...
2. **Shares 앱 개선** - 검색 기능, 태그 시스템, 드래그 앤 드롭 업로드
3. **OAuth 소셜 로그인** - GitHub + Google OAuth 2.0 구현 (포트폴리오)
4. **테스트 커버리지 확대** - API 엔드포인트 및 서비스 레이어 단위 테스트
5. **성능 최적화** - 서비스 레이어 기반 쿼리 최적화 및 캐싱
